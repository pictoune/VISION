...
FMatrix<float,3,3> eight_pt_algo (vector<Match>& sampled_matches, bool refinement){
    ...
    for(int i = 0; i < nb_eq; i++){
        pt_1[0] = sampled_matches[i].x1; pt_1[1] = sampled_matches[i].y1; pt_1[2] = 1;
        pt_2[0] = sampled_matches[i].x2; pt_2[1] = sampled_matches[i].y2; pt_2[2] = 1;

        // normalization
        pt_1 = N*pt_1;
        pt_2 = N*pt_2;

        float x1 = pt_1[0]; float y1 = pt_1[1];
        float x2 = pt_2[0]; float y2 = pt_2[1];
...
vector<int> find_inliers(vector<Match> matches, FMatrix<float,3,3> F, const float distMax){
    ...
    for(size_t i=0; i < matches.size(); i++){
        pt_1[0] = matches[i].x1; pt_1[1] = matches[i].y1; pt_1[2] = 1;
        pt_2[0] = matches[i].x2; pt_2[1] = matches[i].y2; pt_2[2] = 1;

        pt[0] = matches[i].x1; pt[1] = matches[i].y1; pt[2] = 1;
...
void displayEpipolar(Image<Color> I1, const FMatrix<float,3,3>& F) {
    while(true) {
        ...
        if(x <= I1_width){
            ...
            left_pt[0] = I1_width; right_pt[0] = I1_width*2;
        }
        // the user clicked in the right image
            ...
            left_pt[0] = 0; right_pt[0] = I1_width;
        }
