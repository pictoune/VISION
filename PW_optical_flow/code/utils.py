...
def display_results(start,stop,step,best_flow,errors,involved_parameter,best_parameters,dataset_name):
    ...
    if(involved_parameter == "window's size"):
        quiver(best_flow["Angular error"], "Flow obtained after an optimization based on angular error and with a window of size " + str(best_parameters["Angular error"]),scale=False)
        quiver(best_flow["Angular error"], "Normalized flow obtained after an optimization based on angular error and with a window of size " + str(best_parameters["Angular error"]),scale=True)
    elif(involved_parameter == "alpha"):
        quiver(best_flow["Angular error"], r"Flow obtained after an optimization based on angular error and with $\alpha$ = " + str(round(best_parameters["Angular error"],6)),scale=False)
        quiver(best_flow["Angular error"], r"Normalized flow obtained after an optimization based on angular error and with $\alpha$ = " + str(round(best_parameters["Angular error"],6)),scale=True)
        
    
    color_map = middlebury.computeColor(best_flow['Angular error'])
    ...
    if(involved_parameter == "window's size"):
        quiver(best_flow["EndPoint error"], "Flow obtained after an optimization based on EndPoint Error and with a window of size " + str(best_parameters["EndPoint error"]),scale=False)
        quiver(best_flow["EndPoint error"], "Normalized flow obtained after an optimization based on EndPoint Error and with a window of size " + str(best_parameters["EndPoint error"]),scale=True)
    elif(involved_parameter == "alpha"):
        quiver(best_flow["EndPoint error"], r"Flow obtained after an optimization based on EndPoint Error and with $\alpha$ = " + str(best_parameters["EndPoint error"]),scale=False)
        quiver(best_flow["EndPoint error"], r"Normalized flow obtained after an optimization based on EndPoint Error and with $\alpha$ = " + str(best_parameters["EndPoint error"]),scale=True)

    
    color_map = middlebury.computeColor(best_flow["EndPoint error"])
    plt.imshow(color_map)
    
    if(involved_parameter == "window's size"):
        plt.title("Color map obtained after an optimization based on EndPoint Error and with a window of size " + str(best_parameters["EndPoint error"]))
    if(involved_parameter == "alpha"):
        plt.title(r"Color map obtained after an optimization based on EndPoint Error and with $\alpha$ = " + str(round(best_parameters["EndPoint error"],6)))
    ...
    if(involved_parameter == "window's size"):
        quiver(best_flow["Relative norm error"], "Flow obtained after an optimization based on relative norm error and with a window of size " + str(best_parameters["Relative norm error"]),scale=False)
        quiver(best_flow["Relative norm error"], "Normalized flow obtained after an optimization based on relative norm error and with a window of size " + str(best_parameters["Relative norm error"]),scale=True)
    elif(involved_parameter == "alpha"):
        quiver(best_flow["Relative norm error"], r"Flow obtained after an optimization based on relative norm error and with $\alpha$ = " + str(best_parameters["Relative norm error"]),scale=False)
        quiver(best_flow["Relative norm error"], r"Normalized flow obtained after an optimization based on relative norm error and with $\alpha$ = " + str(best_parameters["Relative norm error"]),scale=True)
    
    color_map = middlebury.computeColor(best_flow["Relative norm error"])
    
    plt.imshow(color_map)
    
    if(involved_parameter == "window's size"):
        plt.title("Color map obtained after an optimization based on relative norm error and with a window of size " + str(best_parameters["Relative norm error"]))
    if(involved_parameter == "alpha"):
        plt.title(r"Color map obtained after an optimization based on relative norm error and with $\alpha$ = " + str(round(best_parameters["Relative norm error"],6)))
