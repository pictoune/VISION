...
// Associate a unique node number to a given triplet (x,y,disparity)
int get_node_nb(int ny, int nd, int i, int j, int d)
{
    return nd * ny * i + ny * d + j;
}

// Compute the weight of an edge, i.e. the cost of cutting the edge
float compute_weight(float ncc)
{
    float rho;
...
// Add an edge between the source node and nodes of the first layer
void add_source_edges(Graph<int, int, int> &G, int nx, int ny, int nd,
                      const byteImage &I1,
                      const doubleImage &I1M,
                      const byteImage &I2,
                      const doubleImage &I2M, int H, int W, int n,
                      const int INF, double Kp)
{
    float weight;
    double zncc_;
    ...
    for (int i = 0; i < ny; i++)
    {
        for (int j = 0; j < nx; j++)
        {
            printf("\r%2d/100", (int)((i * nx + j + 1) * 100) / (nx * ny));

            if (0 <= i && i + dmin <= (H - 2 * n) / zoom && 0 <= j && j <= (W - 2 * n) / zoom)
            {
                zncc_ = zncc(I1, I1M, I2, I2M, n + zoom * i, n + zoom * j, n + zoom * i + dmin, n + zoom * j);
                weight = compute_weight(zncc_);
            }
            else
                weight = INF;

            G.add_tweights(get_node_nb(ny, nd, i, j, 0), (int)(weight + Kp), 0);
        }
    }
}

// Add edges between nodes of a same layer and between nodes of consecutive layers
void add_intermediate_edges(
    const byteImage &I1,
    const doubleImage &I1M,
    const byteImage &I2,
    const doubleImage &I2M, int nx, int ny,
    int nd, int W, int n, int H,
    const int INF, Graph<int, int, int> &G, double Kp)
{
    int disp, weight;
    double zncc_;
    ...
    for (int i = 0; i < ny; i++)
    {
        for (int j = 0; j < nx; j++)
        {
            ...
            for (int delta_disp = 0; delta_disp < nd - 1; delta_disp++)
            {
                disp = delta_disp + dmin;

                if (i > 0)
                    // Vertical edge between nodes of a same layer
                    G.add_edge(get_node_nb(ny, nd, i, j, delta_disp), get_node_nb(ny, nd, i - 1, j, delta_disp), lambda, lambda);

                if (j > 0)
                    // Horizontal edge between nodes of a same layer
                    G.add_edge(get_node_nb(ny, nd, i, j, delta_disp), get_node_nb(ny, nd, i, j - 1, delta_disp), lambda, lambda);

                if (0 <= i && i + disp <= (H - 2 * n) / zoom && 0 <= j && j <= (W - 2 * n) / zoom)
                {
                    zncc_ = zncc(I1, I1M, I2, I2M, n + zoom * i, n + zoom * j, n + zoom * i + disp, n + zoom * j);
                ...
                // Edge between nodes of two different layers
                G.add_edge(get_node_nb(ny, nd, i, j, delta_disp), get_node_nb(ny, nd, i, j, delta_disp + 1), (int)(weight + Kp), 0);
            }
        }
    }
}

// Add edges between nodes of the last layer and the sink node
void add_sink_edges(int nx, int ny, int nd, int n, int H,
                    const byteImage &I1,
                    const doubleImage &I1M,
                    const byteImage &I2,
                    const doubleImage &I2M,
                    const int INF, Graph<int, int, int> &G, int W, double Kp)
{
    double zncc_, weight;
    ...
    for (int i = 0; i < ny; i++)
    {
        for (int j = 0; j < nx; j++)
        {
            if (0 <= i && i + dmax <= (H - 2 * n) / zoom && 0 <= j && j <= (W - 2 * n) / zoom)
            {
                zncc_ = zncc(I1, I1M, I2, I2M, n + zoom * i, n + zoom * j, n + zoom * i + dmax, n + zoom * j);
                weight = compute_weight(zncc_);
            }
            else
                weight = INF;
            ...
            G.add_tweights(get_node_nb(ny, nd, i, j, nd - 1), 0, (int)(weight + Kp));
        }
    }
    printf("\n");
}
                 ...
                 int nx, int ny, int nd)
{
    ...
    int W = I2.width(), H = I2.height();
    int n = win;
    double Kp = 1 + (nd - 1) * 4 * lambda; // penalty

    G.add_node(nd * nx * ny);

    add_source_edges(G, nx, ny, nd, I1, I1M, I2, I2M, H, W, n, INF, Kp);
    add_intermediate_edges(I1, I1M, I2, I2M, nx, ny, nd, W, n, H, INF, G, Kp);
    add_sink_edges(nx, ny, nd, n, H, I1, I1M, I2, I2M, INF, G, W, Kp);
}
...
doubleImage decode_graph(Graph<int, int, int> &G, int nx, int ny, int nd)
{
    ...
    for (int y = 0; y < ny; y++)
        for (int x = 0; x < nx; x++)
        {
            ...
            {
                delta_disp++;
            } while (G.what_segment(get_node_nb(ny, nd, x, y, delta_disp)) == Graph<int, int, int>::SOURCE);

            D(x, y) = dmin + delta_disp;
        }
    return D;
}
